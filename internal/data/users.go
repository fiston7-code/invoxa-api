...
// using the json:"-" struct tag to prevent the Password and Version fields from appearing
// in any output when we encode it to JSON. Also notice that the Password field uses the
...
// RETURNING clause to read them into the User struct after the insert, and return the
// updated struct, in the same way that we did when inserting a movie.
