...
USERNAME=fastvoxa
# Prompt to enter a password for the PostgreSQL fastvoxa user (rather than hard-coding
# a password in this script).
read -p "Enter password for fastvoxa DB user: " DB_PASSWORD
...
# Set up the fastvoxa DB and create a user account with the password entered earlier.
sudo -i -u postgres psql -c "CREATE DATABASE fastvoxa"
sudo -i -u postgres psql -d fastvoxa -c "CREATE EXTENSION IF NOT EXISTS citext"
sudo -i -u postgres psql -d fastvoxa -c "CREATE ROLE fastvoxa WITH LOGIN PASSWORD '${DB_PASSWORD}'"
# Add a DSN for connecting to the fastvoxa database to the system-wide environment
# variables in the /etc/environment file.
echo "FASTVOXA_DB_DSN='postgres://fastvoxa:${DB_PASSWORD}@localhost/fastvoxa'" >> /etc/environment
